...
@export var airflow_overlay_strength: float = 0.58
...
@export var min_cell_heat: float = -220.0
...
func set_heat_view_enabled(enabled: bool) -> void:
	...
	shader_material.set_shader_parameter("airflow_overlay_enabled", enable_airflow_overlay)
...
func cache_contains_heat_source(source: Node2D) -> bool:
	for source_info in heat_sources_cache:
		if source_info.get("node", null) == source:
			return true
	return false
...
func inject_source_heat(delta: float) -> void:
	...
	metric_sources_processed += source_count_to_process
	metric_processed_per_step_last = source_count_to_process
	source_processing_ratio_last = float(source_count_to_process) / float(max(cached_source_count, 1))
	...
	for offset in source_count_to_process:
		var source_index: int = (source_batch_cursor + offset) % cached_source_count
		...
		var source_heat: float = float(source.call("get_heat_value"))
		...
		var radius_pixels: float = max(float(source.call("get_heat_radius")), min_cell_radius)
		var radius_x := int(ceil(radius_pixels * inv_cell_x))
		var radius_y := int(ceil(radius_pixels * inv_cell_y))
		radius_x = max(radius_x, 1)
		radius_y = max(radius_y, 1)
		...
		if cooling_capacity > 0.0:
			...
			var cooling_boost: float = 1.0 + clamp(-intake_heat_sample / max(max_visual_heat, 0.001), 0.0, 0.5)
			cooling_term = cooling_capacity * delta * cooling_boost * processing_scale
		...
		for k in range(kernel_offset_x.size()):
			...
			var intake_weight: float = side_emission_ratio + (directional_span * sample_directional_power(intake_alignment))
			var radial_falloff: float = kernel_falloff[k]

			var flow_delta_x: float = (airflow_vec_cells.x * directional_weight) - (intake_vec_cells.x * intake_weight)
			var flow_delta_y: float = (airflow_vec_cells.y * directional_weight) - (intake_vec_cells.y * intake_weight)
	...
	source_batch_cursor = (source_batch_cursor + source_count_to_process) % cached_source_count
...
func run_parity_probe_async(duration_seconds: float = 1.0) -> Dictionary:
	...
	var cell_count: float = float(max(optimized_heat.size(), 1))
	var result := {
		"duration_seconds": duration_seconds,
		"requested_steps": requested_steps,
		"steps": steps,
		"heat_mean_abs_error": heat_abs_sum / cell_count,
		"heat_max_abs_error": heat_max_abs,
		"airflow_mean_abs_error": flow_abs_sum / cell_count,
...
func upload_airflow_texture() -> void:
	...
	for y in HEATMAP_HEIGHT:
		for x in HEATMAP_WIDTH:
			...
			var flow_x_normalized: float = clamp((airflow_visual_x[index] / flow_range) * 0.5 + 0.5, 0.0, 1.0)
			var flow_y_normalized: float = clamp((airflow_visual_y[index] / flow_range) * 0.5 + 0.5, 0.0, 1.0)
			var magnitude: float = clamp(Vector2(airflow_visual_x[index], airflow_visual_y[index]).length() / flow_range, 0.0, 1.0)
