...
var server_power_cost = 0.00035  # Internal cost per handled request-equivalent
...
@export var reputation_decay_per_second: float = 1.6
@export var reputation_threshold_drop_start: float = 0.15
@export var reputation_threshold_penalty_per_second: float = 3.5
@export var reputation_recovery_per_second: float = 0.4
...
@export var revenue_per_valid_handled_request: float = 0.001
@export var cooler_overdrive_power_cost: float = 1.5
...
func _update_reputation_and_failure(delta: float) -> void:
	...
	if servers_active and incoming_total_rps > 0.0:
		var base_decay: float = dropped_ratio * reputation_decay_per_second * sim_delta
		...
		datacenter_reputation -= base_decay + threshold_decay

		if dropped_ratio <= reputation_recovery_drop_ratio_max:
			datacenter_reputation += reputation_recovery_per_second * sim_delta
	else:
...
func _apply_network_cable_flow_utilization(server_flow_map: Dictionary) -> void:
	...
	for segment in tree.get_nodes_in_group("cable_segments"):
		...
		if segment.has_method("set_traffic_load_rps"):
			...
			var capacity_rps := 1.0
			if segment.has_method("get_max_traffic_capacity_rps"):
				capacity_rps = max(float(segment.call("get_max_traffic_capacity_rps")), 1.0)
